...
export default function LoginPage() {
  ...
  return (
    <div className="flex min-h-[700px] flex-col items-center justify-center bg-cream-50 py-12">
      <div className="w-full max-w-md space-y-8 p-8 bg-white rounded-lg shadow-lg border border-border">

        {/* Tabs */}
        <div className="flex border-b border-gray-200">
          <button
            className={`flex-1 pb-4 text-center font-bold text-sm uppercase tracking-wider transition-colors ${activeTab === 'login' ? 'text-gold-600 border-b-2 border-gold-500' : 'text-gray-400 hover:text-gray-600'}`}
            onClick={() => setActiveTab('login')}
          >
            Sign In
          </button>
          <button
            className={`flex-1 pb-4 text-center font-bold text-sm uppercase tracking-wider transition-colors ${activeTab === 'register' ? 'text-gold-600 border-b-2 border-gold-500' : 'text-gray-400 hover:text-gray-600'}`}
        ...
        {activeTab === 'login' && (
          <div className="space-y-6">
            <div className="text-center">
              <h2 className="text-2xl font-black text-charcoal-900">Welcome Back</h2>
              <p className="mt-2 text-sm text-muted-foreground">Access your order history and profile</p>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="block w-full rounded-md border-0 py-2 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-gold-500 sm:text-sm sm:leading-6 px-3"
                ...
                <div className="text-center">
                  ...
                  {stateLogin.error.includes("Email not confirmed") && (
                    <p className="text-xs text-gray-500">
                      Verify your email or <button type="button" className="text-gold-600 font-bold hover:underline" onClick={() => document.getElementById('resend-btn')?.click()} disabled={!captchaToken}>resend confirmation</button>.
                      {!captchaToken && <span className="block text-[10px] text-gray-400 mt-1">(Please verify captcha above first)</span>}
                    </p>
                  )}
                </div>
              )}

              <Button type="submit" disabled={isPendingLogin || !captchaToken} className="w-full bg-gold-500 text-black hover:bg-gold-600 font-bold h-10 disabled:opacity-50">
        ...
        {activeTab === 'register' && (
          <div className="space-y-6">
            <div className="text-center">
              <h2 className="text-2xl font-black text-charcoal-900">Join Us</h2>
              <p className="mt-2 text-sm text-muted-foreground">Create an account to track orders</p>
            </div>
            ...
            {stateSignup?.success ? (
              <div className="text-center py-8">
                ...
                <Button
                  onClick={() => {
                    // Reset states to switch to login
                    setActiveTab('login');
                    // Ideally we should reset stateSignup here but useActionState doesn't expose reset.
                    // Reloading or just switching tab is okay for now.
              ...
              <form action={signupAction} className="space-y-6">
                ...
                <Button
                  ...
                  className="w-full bg-charcoal-900 text-white hover:bg-charcoal-800 font-bold h-10 disabled:opacity-50"
                >
